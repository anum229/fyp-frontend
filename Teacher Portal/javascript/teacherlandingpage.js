...
// Create meeting notification message for teachers
function createMeetingNotification(meeting) {
    const timeString = formatMeetingTime(meeting.startTime, meeting.endTime);
    let participants = `Group ${meeting.groupId} members`;
    
    if (meeting.participants.coAdvisor && meeting.participants.coAdvisor.name) {
        participants += ` and ${meeting.participants.coAdvisor.name}`;
    }
    
    return {
        subject: `Meeting Scheduled`,
        body: `You have a meeting with ${participants} at ${meeting.venue} on ${timeString}.`,
        createdAt: meeting.createdAt,
        type: 'meeting'
    };
}

// Fetch and display meeting notifications for teachers
...
// Display all notifications in sorted order (newest first)
...
function initializeApp() {
    ...
    // Determine which page to load - always keyinsightsteacher for fresh logins
    const pageToLoad = isFreshLogin ? 'keyinsightsteacher' : 
                      (localStorage.getItem('lastVisitedPage') || 'keyinsightsteacher');
    
    loadContent(pageToLoad);
    
    // Setup side panel navigation
    const sidePanelItems = {
        'teacher-landing': 'keyinsightsteacher',
        'submissionsandfeedback': 'submissionsandfeedback',
